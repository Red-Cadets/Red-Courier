...
from tgbot.models.chat import Chat
...
class ChatTG(Chat):
    ...
    def link(self) -> str | None:
        if self.is_private:
            return f"@{self.info}"
        return hlink(self.info, self.link if self.link else self.url)

    @property
    def url(self) -> str:
        return f"tg://user?id={self.id}"

    def receiver(self, to: str) -> int | None:
        config: Config = self.bot.get("config")
        if to == 'MAIN':
            return int(config.tg_bot.main_chat_id)
        elif to == 'DEV':
            return int(config.tg_bot.dev_chat_id)
        else:
            return int(to)
